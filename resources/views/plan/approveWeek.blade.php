                        ...
                        <tbody>
                            ...
                            @if (!$workWeek->isEmpty())
                                @foreach ($workWeek as $value)
                                    ...
                                    @elseif(
                                        ($value->status == 2 && ($user['position_id'] == 5 || $user['position_id'] == 6)) ||
                                            ($value->status == 2 && $value->responsibility == $user['name']))
                                        <tr id="row-{{ $value->id }}">
                                            <td class="col1" style="text-align: center;">{{ $stt++ }}</td>
                                            <td class="left-align"> {{ $value->categoryWeek }} </td>
                                            <td style="white-space: normal" class="left-align  col3">
    ...
    $(document).ready(function() {
        $('.duyettruongnhom').click(function() {
            ...
            $.ajax({
                ...
                success: function(
                data) { // Hàm xử lý khi nhận được phản hồi thành công từ server
                    $('#row-' + id).fadeOut();
                },
