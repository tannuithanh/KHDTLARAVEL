...
<div class="col-lg-12">
    <div class="card">
        <div class="card-body">
            <h1 class="card-title">Danh sách công việc</h1>
                        ...
                        @foreach ($workmonths as $key => $workmonth)
                            <tr id="row-{{ $workmonth->id }}"> 
                                ...
                                <td style="text-align:center; vertical-align: middle;">
                                    @if ($workmonth->responsibility == $user['name'] && $workmonth->status == 3)
                                    <a href="{{ route('editWorkMonth.get', $workmonth->id) }}" class="btn btn-outline-secondary btn-sm edit" title="Sửa"><i class="fas fa-pencil-alt"></i></a>
                                    <button type="button" class="btn btn-outline-danger btn-sm delete ri-delete-bin-line" title="Xóa" data-dialog="dialog-{{ $workmonth->id }}"></button>
                                    @endif
                                </td>
                            </tr>
