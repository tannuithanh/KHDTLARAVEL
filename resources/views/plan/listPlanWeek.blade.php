...
<style>
    ...
    @media print {
        ...
        .col1, .col2, .col3, .col4, .col5, .col6, .col7, .col20 {
        font-size: 19px;
...
<div class="col-12">
    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-body">
                <h4 class="card-title">Danh sách công việc <a href="{{ route('chooseDate.get') }}"
                    ...
                    <table id="my-table"  class="table table-editable table-nowrap align-middle table-edits ">
                        <thead>
                            ...
                            <tr>
                                    ...
                                    <th  style="text-align:center ;" class="table-header">Thứ 2<p>({{$dates[0]}})</p></th>
                                    <th  style="text-align:center ;" class="table-header">Thứ 3<p>({{$dates[1]}})</p></th>
                                    <th  style="text-align:center ;" class="table-header">Thứ 4<p>({{$dates[2]}})</p></th>
                                    <th  style="text-align:center ;" class="table-header">Thứ 5<p>({{$dates[3]}})</p></th>
                                    <th  style="text-align:center ;" class="table-header">Thứ 6<p>({{$dates[4]}})</p></th>
                                    <th  style="text-align:center ;" class="table-header">Thứ 7<p>({{$dates[5]}})</p></th>
                                    <th  style="text-align:center ;" class="table-header">Chủ nhật<p>({{$dates[6]}})</p></th>
                                    ...
                                    @if ($value->startdate >= $start && $value->startdate < $end)
                                        <tr>
                                            @if (Session::has('hack') && session('hack') == $value->id)
                                                <td class="col1" style="text-align: center; background-color: #00802c3b">{{ $stt++ }}</td>
                                                <td class="col2" style=" background-color: #00802c3b" class="left-align"> {{ $value->categoryWeek }}</td>
                                                <td class="col3" style=" background-color: #00802c3b; white-space: normal" class="left-align"> {{ $value->describeWeek }}</td>
                                                ...
                                                @if ($mydate < $value->startdate && $value->status == 0)<td class="hidden-column" style="color:#ffffff;text-align: center; background-color:rgb(133, 58, 219)">Chưa đến hạng </td>
                                                @elseif($mydate <= $value->enddate && $value->status == 0)
                                                    <td class="hidden-column col6" style="text-align: center; background-color:yellow"> Đang thực hiện </td>
                                                @elseif ($mydate > $value->enddate && $value->status == 0)
                                                    <td class="hidden-column col6"style="color:#ffffff; text-align: center; background-color:red">Trễ kế hoạch </td>
                                                @elseif ($value->status == 1)
                                                    <td class="hidden-column col6" style="color:#ffffff; text-align: center; background-color:green">Hoàn thành </td>
                                                @endif
                                                <td style="text-align: center; background-color: #00802c3b" class="hidden-column col7"> {{ $value->note }}</td>
                                            @else
                                                ...
                                                @elseif ($mydate < $value->startdate && $value->status == 0)
                                                    <td class="hidden-column col6"
                                                        ...
                                                        Chưa đến hạng </td>
                                                @elseif($mydate <= $value->enddate && $value->status == 0)
                                                    <td class="hidden-column col6" style="text-align: center; background-color:yellow"> Đang thực
                                                        hiện </td>
                                                @elseif ($mydate > $value->enddate && $value->status == 0)
                                                    <td class="hidden-column col6"
    ...
    $(document).ready(function() {
      $(".lyDoBtn").click(function() {
        ...
        Swal.fire({
          title: 'Nhập lý do',
          html:
            '<textarea id="lydo" class="swal2-textarea" placeholder="Nhập lý do"></textarea>',
          focusConfirm: false,
          showCancelButton: true,
          ...
          if (result.isConfirmed) {
            ...
            $.ajax({
              ...
              success: function(response) {
                $('.lyDoBtn[data-item-id="'+itemId+'"]').css('display', 'none');
              },
    ...
    $(document).on('click', '.reason', function() {
        ...
        Swal.fire({
            ...
            if (confirmAction === 'update') {
                ...
                success: function(response) {
                    $('.reason[data-reason-id="'+itemId+'"]').css('display', 'none');
...
// ------------------------------------------ JS IN TABLE --------------------------------------------------//      
    ...
    <script>
        const formattedDateStart = "{{ $formattedDateStart }}"; // đưa biến formattedDateStart từ PHP vào JavaScript
            ...
            const team = "{{ $user->team_id }}"; // đưa biến team từ PHP vào JavaScript
    ...
    $('#preview-btn').click(function() {
        ...
        if (result.isConfirmed) {
            ...
            const printCSS = '<style type="text/css"> table.print-table th, table.print-table td {border: 1px solid black; font-weight: bold; }</style>';

            document.body.innerHTML = '<html><head><title></title><style>@media print { @page {size: a4 landscape; margin: 0;}table.print-table {width: 80%;max-width: 800%;}}</style></head><body>' + printContents + '</body></html>';
